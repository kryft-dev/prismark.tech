...
Mobile route types are generated by the Expo dev server, so on a fresh
clone `pnpm typecheck` fails for mobile until `pnpm dev` or
`pnpm --filter @prismark/mobile start` has run once.
